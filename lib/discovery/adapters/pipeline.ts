...
//
// Runs ONE discovery_sources row end to end:
//   fetch -> normalize -> dedupe/diff -> upsert into `products` ->
//   match/queue supplier -> mark anything no longer returned as
//   'unavailable' -> log the run
//
// Only ever touches rows it created itself (matched by discovery_source_id
// + external_ref_id). Never touches listing_source='internal' rows.
//
// VERIFY BEFORE FIRST RUN: this assumes `products.price` and
// `products.quantity` accept the same shape your existing addProduct()
// form writes. Adjust the two lines marked VERIFY if not.
...
export async function runDiscoverySource(
  ...
  try {
      ...
      if (existing) {
        ...
        await supabase
          ...
          .update({
            ...
            price: item.price, // VERIFY: matches your addProduct() price format
            unit: item.unit,
    ...
    // ── Removal detection ──────────────────────────────────────────────
    // Anything previously imported from this source that DIDN'T show up in
    // this run gets marked 'unavailable' — kept visible with a note, not
    // deleted or hidden, per how you want this handled.
...
/** Runs every enabled source. Called by /api/discovery/import. */
