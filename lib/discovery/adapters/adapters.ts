...
//
// This is the adapter your first connected source will use. Once you send
// me the actual endpoint + docs, the only thing that needs configuring is
// the `discovery_sources` row's `config.fieldMap` — this code doesn't
// change per source.
...
// Normalizes whatever a source calls its stock status into our three
// values. Extend this map as real sources send different vocab.
function normalizeAvailability(raw: unknown): Availability {
  if (raw == null) return "available"; // absence of a field isn't "unavailable" — assume available unless told otherwise
