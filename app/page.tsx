...
export default function HomePage() {
  ...
  const productTabs = ["All", "Agriculture", "Food", "Livestock", "Machinery", "Fertilizer"];
  ...
  // Stop the button's own click from also triggering the card-wide "go to details" link
  ...
  const messageBtnClass =
    "flex-1 text-center bg-[#f0faf4] text-[#2e8b5a] border border-[#2e8b5a] py-2.5 rounded-xl text-xs sm:text-sm font-semibold hover:bg-[#2e8b5a] hover:text-white transition";

  const buyBtnClass =
    "flex-1 bg-[#2e8b5a] text-white py-2.5 rounded-xl text-xs sm:text-sm font-semibold hover:bg-[#1a4731] transition disabled:bg-gray-300 disabled:cursor-not-allowed shadow-sm";

  return (
    <div className="min-h-screen bg-[#f0faf4]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4">

        {/* Category tabs — only the active tab gets the underline, others plain */}
        <section className="-mx-4 sm:-mx-6 bg-white border-b border-gray-100 mb-5">
          <div className="px-4 sm:px-6 overflow-x-auto">
            <div className="flex items-center gap-6 min-w-max">
              {productTabs.map(function (tab, index) {
                const isActive = (index === 0 && selectedCategory === "") || selectedCategory === tab;
                return (
                  <button
                    key={tab}
                    onClick={function () { setSelectedCategory(index === 0 ? "" : tab); }}
                    className={"pb-2 pt-3 text-sm font-bold whitespace-nowrap " + (isActive ? "text-black border-b-4 border-black" : "text-gray-500")}
                  >
                    {tab}
                  </button>
                );
        ...
        {/* Empty State — different message when a specific category has nothing yet */}
        {!loading && filteredProducts.length === 0 && (
          <div className="bg-white rounded-3xl text-center py-24 border border-gray-100 shadow-sm">
            ...
            <h3 className="text-2xl font-bold text-gray-800 mb-2">
              {selectedCategory ? "No products in " + selectedCategory + " yet" : "No products found"}
            </h3>
            <p className="text-gray-500 mb-8 max-w-xs mx-auto">
              {selectedCategory
                ? "Nothing has been listed in this category yet. Check back soon or browse another category."
        ...
        {/* Products Grid — whole card links to details, Message/Order stay independent */}
              ...
              >
                <div className="h-32 sm:h-60 bg-gradient-to-br from-[#f0faf4] to-[#e8f5f0] relative overflow-hidden">
                  {product.image ? (
                    ...
                    <div className="h-full flex flex-col items-center justify-center text-gray-300">
                      <svg className="w-8 h-8 sm:w-12 sm:h-12 mb-2 opacity-20" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="p-3 sm:p-6 flex flex-col flex-1">
                  <div className="mb-2 sm:mb-4">
                    ...
                    <p className="text-[#2e8b5a] font-black text-base sm:text-2xl">₦{product.price}</p>
                  </div>
                  ...
                  <div className="space-y-1.5 sm:space-y-3 mb-3 sm:mb-6 flex-1">
                    ...
                    <div className="flex items-center gap-2 sm:gap-2.5 text-gray-600 text-xs sm:text-sm">
                      ...
                      <div className="min-w-0">
                        <p className="hidden sm:block text-[10px] text-gray-400 uppercase font-bold leading-none mb-1">Seller</p>
                        <p className="font-bold text-gray-800 leading-none truncate">{product.seller}</p>
                      </div>
                    </div>
                  ...
                  {/* Message + Order — stop the click so it doesn't also trigger the card's own link */}
                  <div className="flex gap-1.5 sm:gap-2 pt-3 sm:pt-4 border-t border-gray-50">
                    <a
                    ...
                    <button
                      onClick={function (e) { stopCardNav(e); handleBuy(product); }}
