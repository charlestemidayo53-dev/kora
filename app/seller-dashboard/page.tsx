...
export default function SellerDashboardPage() {
  ...
  const backBtnClass = "flex items-center gap-2 text-gray-500 hover:text-[#2e8b5a] transition text-sm font-medium";
  const addBtnClass = "bg-[#2e8b5a] text-white px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-[#1a4731] transition";
  const editBtnClass = "flex-1 text-center bg-[#f0faf4] text-[#2e8b5a] py-2.5 rounded-xl text-sm font-semibold hover:bg-[#2e8b5a] hover:text-white transition";
  const deleteBtnClass = "flex-1 text-center bg-red-50 text-red-500 py-2.5 rounded-xl text-sm font-semibold hover:bg-red-500 hover:text-white transition disabled:opacity-50 disabled:cursor-not-allowed";
    ...
    <div className="min-h-screen bg-[#f0faf4]">
      ...
      <div className="max-w-6xl mx-auto px-6 py-10">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-black text-[#1a4731] mb-1">Seller Dashboard</h1>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {products.map(function (product) {
            return (
              ...
              >
                <div className="h-48 bg-gradient-to-br from-[#f0faf4] to-[#e8f5f0] relative overflow-hidden">
                  {product.image ? (
                    <img
                      src={product.image}
                      alt={product.name}
                  ...
                  {product.category && (
                    <span className="absolute top-3 left-3 bg-white/90 backdrop-blur-sm text-[#2e8b5a] text-xs font-bold px-3 py-1 rounded-full shadow-sm">
                      {product.category}
                    </span>
                  )}
                </div>
                ...
                <div className="p-5 flex flex-col flex-1">
                  <h2 className="text-lg font-bold text-gray-900 mb-1 line-clamp-1">{product.name}</h2>
                  <p className="text-[#2e8b5a] font-black text-xl mb-1">N{product.price}</p>
                  {product.location && (
